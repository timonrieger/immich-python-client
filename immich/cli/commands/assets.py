...
def check_bulk_upload(
    ctx: typer.Context,
    assets: list[str] = typer.Option(..., "--assets", help="key=value pairs (repeatable); e.g. key1=value1,key2=value2"),
) -> None:
    """Check bulk upload
...
def delete_bulk_asset_metadata(
    ctx: typer.Context,
    items: list[str] = typer.Option(..., "--items", help="key=value pairs (repeatable); e.g. key1=value1,key2=value2"),
) -> None:
...
Docs: https://api.immich.app/endpoints/assets/getAllUserAssetsByDeviceId
    ...
    client = ctx.obj['client']
    result = run_command(client, client.assets, 'get_all_user_assets_by_device_id', **kwargs)
    format_mode = ctx.obj.get('format', 'pretty')
    print_response(result, format_mode)
